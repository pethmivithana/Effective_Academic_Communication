import '../unit_model.dart';

final UnitModel unit4_2 = UnitModel(
  unitIndex: 18,
  unitName: "Vocabulary Learning Strategies",
  introductionText: """
Hello everyone!
Have you ever found yourself struggling to find the right word when speaking English? You know what you want to say—but the vocabulary just doesn’t come to mind. This often happens because of a limited vocabulary range. Now imagine having a rich and vivid vocabulary—it would naturally make your speech more fluent and expressive.
In today’s lesson, we’ll explore practical and enjoyable strategies for building your vocabulary. These methods can help you not only speak and write more effectively but also become a more confident English user.
Lesson Objectives
By the end of this lesson, you will be able to:
1.	Identify various vocabulary learning strategies suitable for academic and everyday use.
2.	Apply appropriate strategies to enhance your vocabulary for academic and professional communication.
3.	Develop a personalized vocabulary learning plan based on your preferences and goals.
""",

  preClassActivityDescription: """
Step 1:
Click the link below and complete the vocabulary quiz to assess your current vocabulary level.

Step 2:
Take a screenshot of your final results page and upload it to the platform.
 Note: This isn’t for grading purposes, but to confirm that you've completed the activity.

""",
  preClassActivityLink: "https://www.merriam-webster.com/games/vocabulary-quiz ",
  preClassActivityUploadLink: "https://example.com/upload-practice-activity",

  practiceActivityDescription1: """ 
Step 1:
Download the Duolingo app (if you haven’t already).

Step 2:
Set your learning language to English and choose a 10-minute daily goal.

Step 3:
Complete at least 3 vocabulary exercises on Duolingo and take screenshots of the results to upload as proof.

Step 4:
Write down 5 new words you learned from the exercises. Try to use them in your own sentences!
""",
  practiceUploadLink:"https://example.com/upload-practice-activity",

  practiceActivityDescription2: """
Step 1:
Access the vocabulary quiz on Computer and Technology terms through the following link

Step 2:
Complete the quiz and check your answers.

Step 3:
Take a screenshot of your final score/result page after checking your answers and upload it as proof of completion.
""",
  practiceActivityLink2:"https://www.englishclub.com/vocabulary/computer-quiz.php#google_vignette ",
  practiceUploadLink2: "https://example.com/upload-practice-activity",

  instructionsText: """
Once you’ve completed the pre-class activity, watch the recorded mini-lesson provided. It explains various strategies and apps you can use to expand your vocabulary, both for general and academic purposes. 
""",

  instructionVideoId: "8laYlyAFd0o",

  summary: """
In this lesson, we learned that vocabulary expansion is a process that requires consistency and the right strategies. Whether it’s through mobile apps like Duolingo, quizzes, games, or interactive activities, you now have a toolkit to help you become more fluent, articulate, and confident in English communication.""",

  inClassActivity: """
In class, we will conduct a series of interactive activities to reinforce the strategies you’ve learned. See you! 
""",

  quizQuestions: [
    QuizQuestion(
      question: "What is the best definition for the word “articulate”?",
      options: [
        "Shy and reserved",
        "Able to express thoughts clearly and effectively",
        "Always quiet",
        "Difficult to understand"
      ],
      correctOptionIndex: 1,
    ),
    QuizQuestion(
      question: "Which of the following is a synonym for “vivid”?",
      options: [
        "Boring",
        "Bright",
        "Weak",
        "Cold"
      ],
      correctOptionIndex: 1,
    ),
    QuizQuestion(
      question: "The word “comprehend” is closest in meaning to:",
      options: [
        "Speak",
        "Translate",
        "Understand",
        "Forget"
      ],
      correctOptionIndex: 2,
    ),
    QuizQuestion(
      question: "If someone is “fluent” in a language, what does that mean?",
      options: [
        "They can write only",
        "They can speak easily and confidently",
        "They know all the grammar rules",
        "They struggle to communicate"
      ],
      correctOptionIndex: 1,
    ),
    QuizQuestion(
      question: "Which of these is an example of a learning strategy?",
      options: [
        "Watching TV",
        "Using flashcards",
        "Sleeping",
        "Eating snacks"
      ],
      correctOptionIndex: 1,
    ),
    QuizQuestion(
      question: "What does “synonym” mean?",
      options: [
        "A word that has the opposite meaning",
        "A word from another language",
        "A word with the same or similar meaning",
        "A verb"
      ],
      correctOptionIndex: 2,
    ),
    QuizQuestion(
      question: "If something is “challenging,” it is:",
      options: [
        "Easy",
        "Boring",
        "Difficult but interesting",
        "Short"
      ],
      correctOptionIndex: 2,
    ),
    QuizQuestion(
      question: "What does the word “retain” mean in vocabulary learning?",
      options: [
        "Forget quickly",
        "Remember for a long time",
        "Spell correctly",
        "Use only in writing"
      ],
      correctOptionIndex: 1,
    ),
    QuizQuestion(
      question: "What is the opposite of “limited vocabulary”?",
      options: [
        "Poor vocabulary",
        "Rich vocabulary",
        "Foreign vocabulary",
        "Old vocabulary"
      ],
      correctOptionIndex: 1,
    ),
    QuizQuestion(
      question: "The phrase “in context” means:",
      options: [
        "Without using examples",
        "In real situations with meaning",
        "From the dictionary",
        "Only in writing"
      ],
      correctOptionIndex: 1,
    ),
  ],
);
